...
export function WeatherLayout() {
  ...
  const [cityWeather, setCityWeather] = useState(getFromStorage('CURRENT_WEATHER') || null)

  const [forecast, setForecast] = useState<FetchedWeather['forecast'] | null>([])

  const { data, dataUpdatedAt } = useGetWeatherByLatLon(selectedCity.lat, selectedCity.lon, {
    refetchInterval: ms('10m') // 10 minutes
  })
  const { data: forecastData, dataUpdatedAt: forecastUpdatedAt } = useGetForecastWeatherByLatLon(
    selectedCity.lat,
    selectedCity.lon,
    {
  ...
  useEffect(() => {
    if (data) {
      setCityWeather(data)
      setToStorage('CURRENT_WEATHER', data)
    }
  }, [dataUpdatedAt])
    ...
    <>
      <section className='p-2 mx-1 rounded lg:mx-4'>
        <div className='flex items-center justify-between w-full px-1'>
          <h2 className='text-lg font-semibold dark:text-gray-200 font-[balooTamma]'>☂️ Weather</h2>
          <div
        ...
        <div className='grid grid-cols-2 gap-2 p-1 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6'>
          ...
          {forecast?.length
            ? forecast.map(item => <ForecastComponent forecast={item} key={item.temp} />)
            : null}
        </div>
      </section>
